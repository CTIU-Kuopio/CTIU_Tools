...
"%PREFIX%\python.exe" -m pip install -r "%PREFIX%\FileSorter\requirements.txt" >> "%PREFIX%\menuinst_debug.log"

IF EXIST "%PREFIX%\FileSorter\requirements-windows.txt" (
    "%PREFIX%\python.exe" -m pip install -r "%PREFIX%\FileSorter\requirements-windows.txt" >> "%PREFIX%\menuinst_debug.log"
)

SET "PROJECT_ROOT=%PREFIX%\FileSorter"
...
"%PREFIX%\python.exe" "%PREFIX%\FileSorter\include_path.py" --path "%PREFIX%" --files "%PREFIX%\FileSorter\notebook_launcher.json" --keyword "BASE_PATH_KEYWORD" >> "%PREFIX%\menuinst_debug.log"
"%PREFIX%\python.exe" "%PREFIX%\FileSorter\hide_code_cells.py" "%PREFIX%\FileSorter" >> "%PREFIX%\menuinst_debug.log"
...
"%PREFIX%\python.exe" -c "from menuinst.api import install; import os; print(install(os.path.join(r'%PREFIX%', 'FileSorter', 'notebook_launcher.json')))" >> "%PREFIX%\menuinst_debug.log" 2>&1

SET "ARP_KEY=HKCU\Software\Microsoft\Windows\CurrentVersion\Uninstall\FileSorter"
SET "UNINSTALL_EXE=%PREFIX%\Uninstall-FileSorter.exe"
SET "DISPLAY_ICON=%PREFIX%\FileSorter\logo.ico"
SET "DISPLAY_VERSION=0.0.1"
SET "PUBLISHER=CTIU-Kuopio"
echo Registering FileSorter in Windows Apps list >> "%PREFIX%\menuinst_debug.log"
reg add "%ARP_KEY%" /v DisplayName /d "FileSorter" /f >> "%PREFIX%\menuinst_debug.log" 2>&1
